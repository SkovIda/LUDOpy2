# Code from Source: https://github.com/NDurocher/YARAL/blob/main/src/Qlearn.py
# Modified to integrate with ludopy2 (which is modified from ludopy lib)
...
class QPlayer(Player):
    ...
    def load_q_player(self, tableName="Qs.txt", newtbl=False, isTrain=False):
        self.tableName = tableName
        self.LUT = self.Qinit(newtbl)
        self.prev_state = np.zeros([4], dtype=int)
        self.prev_action = 9*np.ones([4], dtype=int)
        ...
        # discount
        self.delta = 0.6
        # self.piece, self.action, self.state = None

        # print(self.LUT)


    def next_move(self, player_i, player_pieces, enemy_pieces, dice, move_pieces):
    ...
    def adjustenemy(self, enemy):
        adj_enemy = np.zeros(np.array(enemy).shape)
